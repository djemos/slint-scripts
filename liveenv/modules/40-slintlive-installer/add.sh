...
SLINTLIVEINSTALLER="$(readlink -f "$(ls -1 "$PKGSDIR"/sli-*.txz "$LOCALDIR"/sli-*.txz 2>/dev/null | head -n 1)")"
if [ -n "$SLINTLIVEINSTALLER" ]; then
  tar xf "$SLINTLIVEINSTALLER" usr/share/applications
  mkdir -p "$RDIR"/home/one/Desktop
  cp usr/share/applications/*.desktop "$RDIR"/home/one/Desktop/
  rm -rf usr
else
  echo "Cannot add $0 because package slint-live-installer is missing in PKGS." >&2
  exit 1
fi
